...
void Peer::handle_response(const boost::system::error_code& err,
  ...
  if (!err || err == asio::error::eof) {
    ...
    while (response_it < m_response.begin() + bytes_transferred) {
      ...
      std::vector<unsigned char> byte_length_buff(response_it, response_it + 4);
      int byte_length = boost::endian::load_big_u32(byte_length_buff.data());
      response_it += 4;
      ...
      try {
        ...
        std::cout << m_base_log + " err: " + err.what() + " " +
      ...
      int message_id = static_cast<unsigned char>(message[0]);
      ...
      if (message_id == static_cast<int>(Message_type::choke)) {
        ...
        std::cout << m_base_log + " Message: INTERESTED\n";
        ...
        std::cout << m_base_log.append(" Message: BITFIELD | size = " +
        ...
        std::cout << m_base_log.append(" Message: UNKNOWN ID[" +
...
Peer::Peer(int id, std::string peer_id, tcp::endpoint& ep,
    ...
    : m_id(id),
      ...
      m_port(ep.port()),
      m_endpoint(ep),
      m_torrent_file(std::move(torrent_file)),
      m_response(1024) {
